...
class GilderUI(CTkWithDnD):
    def __init__(self):
        ...
        self.processing_mode = ctk.StringVar(value="amend")
        self.selected_file_path = None
        self.selected_banks_path = None  # Track custom bank classification Excel path
        ...
        self._build_header()
        self._build_dropzone()
        self._build_banks_dropzone()  # Updated for Excel file classification
        self._build_options()
        self._build_footer()
    ...
    def _build_header(self):
        header_frame = ctk.CTkFrame(self, fg_color="transparent")
        header_frame.pack(pady=(40, 15), fill="x")
    ...
    def _build_dropzone(self):
        ...
        self.drop_frame.pack(pady=(15, 10), padx=80, fill="x")
    ...
    def _build_banks_dropzone(self):
        # Height is exactly 80px (half of 160px) and width constraints match the primary dropzone
        self.banks_drop_frame = ctk.CTkFrame(self, fg_color="white",
                                             border_color="#D2D6DC", border_width=2, corner_radius=15,
                                             cursor="hand2", height=80)
        self.banks_drop_frame.pack(pady=(10, 15), padx=80, fill="x")
        self.banks_drop_frame.pack_propagate(False)

        self.banks_inner_frame = ctk.CTkFrame(self.banks_drop_frame, fg_color="transparent", cursor="hand2")
        self.banks_inner_frame.place(relx=0.5, rely=0.5, anchor="center")

        self.banks_icon = ctk.CTkLabel(self.banks_inner_frame, text="📊", font=("Arial", 22),
                                       text_color="#94A3B8", cursor="hand2")
        self.banks_icon.pack(side="left", padx=(0, 10))

        self.banks_drag_text = ctk.CTkLabel(self.banks_inner_frame,
                                            text="Перетягніть сюди .xlsx класифікації банків або натисніть",
                                            font=("Segoe UI", 13), text_color="#1E293B", cursor="hand2")
        self.banks_drag_text.pack(side="left")

        clickable_elements = [self.banks_drop_frame, self.banks_inner_frame, self.banks_icon, self.banks_drag_text]

        for element in clickable_elements:
            element.bind("<Button-1>", self._browse_banks_file)
            element.bind("<Enter>", self._highlight_banks_dropzone)
            element.bind("<Leave>", self._unhighlight_banks_dropzone)
            element.drop_target_register(DND_FILES)
            element.dnd_bind('<<Drop>>', self._handle_banks_file_drop)
            element.dnd_bind('<<DragEnter>>', self._highlight_banks_dropzone)
            element.dnd_bind('<<DragLeave>>', self._unhighlight_banks_dropzone)
    ...
    # --- Bank Dropzone Actions (Excel Migration) ---
    def _highlight_banks_dropzone(self, event=None):
        self.banks_drop_frame.configure(fg_color="#F1F5F9")

    def _unhighlight_banks_dropzone(self, event=None):
        try:
            x, y = self.winfo_pointerx(), self.winfo_pointery()
            x1 = self.banks_drop_frame.winfo_rootx()
            y1 = self.banks_drop_frame.winfo_rooty()
            x2 = x1 + self.banks_drop_frame.winfo_width()
            y2 = y1 + self.banks_drop_frame.winfo_height()

            if not (x1 <= x <= x2 and y1 <= y <= y2):
                self.banks_drop_frame.configure(fg_color="white")
        except:
            self.banks_drop_frame.configure(fg_color="white")

    def _browse_banks_file(self, event=None):
        file_path = filedialog.askopenfilename(
            title="Оберіть файл класифікації банків (Excel)",
            filetypes=[("Excel Files", "*.xlsx *.xls")]
        )
        if file_path:
            self._update_banks_ui_with_file(file_path)

    def _handle_banks_file_drop(self, event):
        self.banks_drop_frame.configure(fg_color="white")
        file_path = event.data.strip('{}')
        ...
        if file_path.lower().endswith(('.xlsx', '.xls')):
            ...
            self.banks_drag_text.configure(text="Цей тип файлу не підтримується. Оберіть Excel-файл (.xlsx, .xls).",
                                           text_color="red")

    def _update_banks_ui_with_file(self, file_path):
        self.selected_banks_path = file_path
        filename = os.path.basename(file_path)

        self.banks_icon.configure(text="✅", text_color="#2A9D6A")
        self.banks_drag_text.configure(text=f"Класифікація: {filename}", text_color="#1E293B", font=("Segoe UI", 13, "bold"))
        self.banks_drop_frame.configure(border_color="#2A9D6A")

    def _process_file(self):
        if not self.selected_file_path:
            self.drag_text.configure(text="⚠️ Будь ласка, спершу оберіть файл!", text_color="#EAB308")
            return
        if not self.selected_banks_path:
            self.banks_drag_text.configure(text="⚠️ Будь ласка, спершу оберіть файл!", text_color="#EAB308")
            return
        ...
        # # Setup fallbacks if user leaves custom bank configuration file blank
        # default_banks_file = "/Users/illiaknu/Desktop/OSB_Gilder/OSB_Gilder/back/testing/banks.xlsx"
        # banks_file_to_use = self.selected_banks_path if self.selected_banks_path else default_banks_file

        mode = self.processing_mode.get()
        if mode == "amend":
            self.worker_thread = OSBGilder(
                parent_file=self.selected_file_path,
                progress_var=self.progress_var,
                cancel_event=self.cancel_event,
                ind_file="/Users/illiaknu/Desktop/OSB_Gilder/OSB_Gilder/back/script/ind.csv",
                banks_file=self.selected_banks_path
            )
            self.worker_thread.daemon = True
            self.worker_thread.start()
